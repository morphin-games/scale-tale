...
func ready() -> void:
	...
	(platformer_pawn_state.platformer_pawn._controller as PlayerController).kxi_jump_pressed.connect(Callable(func() -> void:
		...
		var force_status : Error = platformer_pawn_state.state_machine.force_state("PPStateUndiving")
		if(force_status == OK):
		...
		push_warning("PPStateActionDive tried to force PPStateUndiving in a PPStateMachine that doesn't have that state!")
	))
