...
func action(args : Array[String] = []) -> Array[String]:
	_terminal.text = ""
	_terminal.set_caret_line(0)
	_terminal.set_caret_column(_terminal.get_line(0).length())
