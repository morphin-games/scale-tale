...
func action(args : Array[String] = []) -> Array[String]:
	if(args.has("-h") or args.has("--help")):
		return [
			"editscn [relative_path] : Open the given scene from a path relative to the current terminal directory.",
			"editscn res://[path]    : Open the given scene from a full path using res://.\n",
		]
	elif(args.size() == 1):
		if(args[0].left(6) == "res://"):
			EditorInterface.open_scene_from_path(args[0])
		else:
			EditorInterface.open_scene_from_path(_terminal.get_terminal_path() + args[0])
