...
func action(args : Array[String] = []) -> Array[String]:
	...
	scene.pack(root)
	if(path.left(6) == "res://"):
		ResourceSaver.save(scene, path + name.to_snake_case() + ".tscn")
		EditorInterface.open_scene_from_path(path + name.to_snake_case() + ".tscn")
	else:
		ResourceSaver.save(scene, _terminal.get_terminal_path() + name.to_snake_case() + ".tscn")
		EditorInterface.open_scene_from_path(_terminal.get_terminal_path() + name.to_snake_case() + ".tscn")
